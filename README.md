...
_Lưu ý: PostgreSQL sẽ chạy trên cổng máy chủ **5433**._
...
## Import Excel templates (Student / Lecturer)

Mẫu dữ liệu để test import nằm tại thư mục `SampleExcels/`:

- `SampleExcels/StudentImportTemplate.csv`
- `SampleExcels/LecturerImportTemplate.csv`

> Lưu ý: UI hiện upload **.xlsx**. Bạn có thể mở file `.csv` bằng Excel/Google Sheets và **Save As / Download as** `.xlsx` rồi upload.

Cột `CourseCodes` hỗ trợ nhiều mã môn, ngăn cách bởi `;` (ví dụ: `PRN222;SWR302`).
